...
class ButtonLink extends LitWithoutShadowDom{
    ...
    _checkAvailabilityProperty() {
        if(!this.hasAttribute('to')) {
            throw new Error(`Atribute "to" harus di terapkan pada element ${this.localName}`);
        }

        if(!this.hasAttribute('content')){
            throw new Error(`Atribute "content" harus di terapkan pada element ${this.localName}`);
        }
    }
